...
### Consume as a git submodule (no registry)
...
inlines every dependency except React. The shipped **`dist/index.css` contains only
the grid's component + token (CSS-variable) styles — it deliberately does NOT ship a
Tailwind utility layer.** A component library that dumped global `.flex`/`.hidden`/etc.
would collide with and override the host app's own utilities. Instead, the grid's
inline Tailwind utility classes are generated by the **consuming app's own Tailwind**
(every house app uses Tailwind), which needs two one-time additions.
...
1. Alias + import the built entry (Vite example):

```ts
// vite.config.ts
resolve: { alias: { '@prdgrid': resolve(__dirname, 'src/vendor/prdgrid/dist') } }
// tsconfig paths: "@prdgrid": ["./src/vendor/prdgrid/dist/index.d.ts"], "@prdgrid/*": ["./src/vendor/prdgrid/dist/*"]
```

```tsx
import { DataGrid } from '@prdgrid';
import '@prdgrid/index.css';   // component + token styles only
```

2. Make the host Tailwind generate the grid's utilities — scan the dist and add the
   `grid-*` tokens (copy the `theme.extend.colors.grid` + `fontSize` blocks from this
   repo's `tailwind.config.ts`):

```js
// tailwind.config.js (host app)
content: ['./src/**/*.{ts,tsx}', './src/vendor/prdgrid/dist/**/*.js'],
theme: { extend: { colors: { grid: { /* …copy from prdgrid tailwind.config.ts… */ } } } }
```
