...
function App() {
  return (
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex items-center gap-0.5 overflow-x-auto">
            {navItems.map(n => (
              <a key={n.id} href={`#${n.id}`}
                className="flex items-center gap-1 whitespace-nowrap rounded-md px-2 py-1 text-[11px] font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-900 transition-colors">
      ...
      {/* Hero */}
      <div className="bg-gradient-to-br from-blue-600 via-indigo-600 to-violet-700 text-white py-14 px-6">
        <div className="mx-auto max-w-[1440px]">
          <h1 className="text-3xl font-black mb-1.5 tracking-tight">@jobtalk/datagrid</h1>
          <p className="text-blue-100 text-base mb-5 max-w-2xl">
            Enterprise-grade React data grid — AG Grid-level features with zero license fees.
            Built for JobTalk.ai's recruiting platform and all internal applications.
          </p>
          <div className="flex flex-wrap gap-2 text-[11px] font-medium">
            {['Sorting', 'Filtering', 'Grouping', 'Aggregation', 'Cell Editing', 'Column Manager',
              'Pinning', 'Persistence', 'CSV / Excel / Image Export', 'Email & Schedule',
              'Dark Mode', 'Custom Themes', 'Pagination', 'Custom Renderers', 'API Control',
              'Density Toggle', 'Status Bar', 'Floating Filters', 'Group Panel'].map(f => (
              <span key={f} className="rounded-full bg-white/15 backdrop-blur-sm px-2.5 py-1 border border-white/10">{f}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1440px] px-6 py-10">
        ...
        <footer className="mt-20 border-t border-gray-200 pt-6 pb-10 text-center">
          <p className="text-sm text-gray-400">@jobtalk/datagrid v0.1.0</p>
